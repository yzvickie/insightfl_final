...
def insertdata_db(host, user, passwd):
    ...
    startup_info = open('startupinfo_list_markets_funding')
    ...
    for row in startup_info:
        try:
            ...
            cur.execute('''INSERT INTO Starup_info
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s)''', (row['id'], row['name'], row['market'], row['quality'], row['angellist_url'], row['logo_url'],row['totalfunding'], row['product_desc']))
        except Exception, e:
            print row, e
